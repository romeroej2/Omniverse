...
namespace Switcharoo.Views
{
    public partial class SwitcharooWindow : Window
    {
        ...
        private void ExportSettings(object sender, RoutedEventArgs routedEventArgs)
        {
            ...
            {
                var url = trello.GetAuthorizationUrl("Switcharoo", Scope.ReadWrite, Expiration.ThirtyDays);
            ...
            try
            {
                ...
                try
                {
                    ...
                    using (var archive = ZipFile.Open($"{name} Bug Report.zip", ZipArchiveMode.Create))
                    {
                        ...
                        archive.CreateEntryFromFile(@"Settings/" + Core.Me.Name + "/Switcharoo/Main_Settings.json", "Main_Settings.json");
                        archive.CreateEntryFromFile("Bug Report Description", "Bug Report Description.txt");
                    }
                }
                catch (Exception e)
                {
                    Logger.SwitcharooLog(e.ToString());
                }
            ...
            {
                Logger.SwitcharooLog("Chances are, you closed the authorization window before you actually authorized Switcharoo to post. Try to re-submit the error report.");
