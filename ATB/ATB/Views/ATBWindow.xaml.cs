...
using Constants = ATB.Utilities.Constants;
...
namespace ATB.Views
{
    public partial class ATBWindow : Window
    {
        ...
        private void TrelloReport(object sender, RoutedEventArgs routedEventArgs)
        {
            var priorAuth = false;
            ...
            {
                var url = trello.GetAuthorizationUrl("ATB", Scope.ReadWrite, Expiration.ThirtyDays);
            ...
            try
            {
                ...
                try
                {
                    ...
                    using (var archive = ZipFile.Open($"ATB Bug Report.zip", ZipArchiveMode.Create))
                    {
                        ...
                        archive.CreateEntryFromFile(@"Settings/" + Constants.Me.Name + "/ATB/Main_Settings.json", "Main_Settings.json");
                    }
                }
                catch (Exception e)
                {
                    var errorWindow = MessageBox.Show(e.ToString(), "There was an error", MessageBoxButton.OK);
                    Logger.ATBLog(e.ToString());
                ...
                trello.Cards.AddAttachment(newBugCard, new FileAttachment($"ATB Bug Report.zip", ($"ATB Bug Report")));
            ...
            {
                var errorWindow = MessageBox.Show(e + "\n\n Chances are, you closed the authorization window before you actually authorized ATB to post. Try to re-submit the error report.", "There was an error", MessageBoxButton.OK);
                Logger.ATBLog(e.ToString());
                Logger.ATBLog("Chances are, you closed the authorization window before you actually authorized ATB to post. Try to re-submit the error report.");
                Core.OverlayManager.AddToast(() => "Error: Re-Submit Error Report.", TimeSpan.FromMilliseconds(3000), Color.FromRgb(110, 225, 214), Colors.White, new FontFamily("High Tower Text Italic"), new FontWeight(), 52);
            }

            cleanup:
            if (File.Exists("Bug Report Description"))
                File.Delete("Bug Report Description");
            if (File.Exists($"ATB Bug Report.zip"))
                File.Delete($"ATB Bug Report.zip");
            if (File.Exists($"ATB Bug Report"))
                File.Delete($"ATB Bug Report");
