...
namespace Syntgoht.Views
{
    public partial class SyntgohtWindow : Window
    {
        ...
        private void ExportSettings(object sender, RoutedEventArgs routedEventArgs)
        {
            ...
            {
                var url = trello.GetAuthorizationUrl("Syntgoht", Scope.ReadWrite, Expiration.ThirtyDays);
            ...
            try
            {
                ...
                try
                {
                    ...
                    using (var archive = ZipFile.Open($"{name} Bug Report.zip", ZipArchiveMode.Create))
                    {
                        ...
                        archive.CreateEntryFromFile(@"Settings/" + Core.Me.Name + $"/Syntgoht/Syntgoht_Hotkeys/{name}_Hotkeys.json", $"{name}_Hotkeys.json");
                        archive.CreateEntryFromFile(@"Settings/" + Core.Me.Name + "/Syntgoht/Main_Settings.json", "Main_Settings.json");
                        archive.CreateEntryFromFile("Bug Report Description", "Bug Report Description.txt");
                    }
                }
                catch (Exception e)
                {
                    Logger.SyntgohtLog(e.ToString());
                }
            ...
            {
                Logger.SyntgohtLog("Chances are, you closed the authorization window before you actually authorized Syntgoht to post. Try to re-submit the error report.");
